...
folderPath='C:\Users\sande\Documents\Imperial\Documents\MCL study\New protocol\SH14\Digitise';
...
% For femur
[gTf0, originF, APAxisF, PDAxisF, MLAxisF] = defineBodyFixedFrameFemur_MCL(F_1_Med, F_2_Lat, F_3_Prox, Right);
grf0 = [originF, 1]';
...
%% Load tracked points
folderPath='C:\Users\sande\Documents\Imperial\Documents\MCL study\New protocol\SH14\Intact\';
kneestate=1;
loadtype='Anterior.csv';
fileName=[folderPath,loadtype];
% Output='C:\Users\Sander\Documents\Imperial\matlab\Files\RB02\RB_test.xlsx';
...
%preallocate arrays
tibiaRxRyRzs=zeros(length(TrackedData.points),3);
femurRxRyRzs=zeros(length(TrackedData.points),3);
...
%preallocate arrays
gTti=cell(size(gTfti,1));
grti=cell(size(gTfti,1));
gTfi=cell(size(gTfti,1));
grfi=cell(size(gTfti,1));
fTt=cell(size(gTfti,1));
frti=cell(size(gTfti,1));
f0rfi=cell(size(gTfti,1));
t0rti=cell(size(gTfti,1));
t0Tti=cell(size(gTfti,1));
f0Tfi=cell(size(gTfti,1));
f0Tt=cell(size(gTfti,1));
ollie_I_=zeros(3,length(TrackedData.points));
ollie_J_=zeros(3,length(TrackedData.points));
ollie_K_=zeros(3,length(TrackedData.points));
ollie_i_=zeros(3,length(TrackedData.points));
ollie_j_=zeros(3,length(TrackedData.points));
ollie_k_=zeros(3,length(TrackedData.points));
ollie_e1_=zeros(3,length(TrackedData.points));
ollie_e2_=zeros(3,length(TrackedData.points));
ollie_e3_=zeros(3,length(TrackedData.points));
ollie_anglestf=zeros(length(TrackedData.points),3);
ollie_XYZtf=zeros(length(TrackedData.points),3);
ollie_beta=zeros(length(TrackedData.points),1);
ollie_IntExt=zeros(length(TrackedData.points),1);
ollie_VarVal=zeros(length(TrackedData.points),1);
ollie_AntPost=zeros(length(TrackedData.points));
ollie_H_=zeros(length(TrackedData.points),3);
ollie_MedLat=zeros(length(TrackedData.points),1);
ollie_ProxDist=zeros(length(TrackedData.points),1);
...
folderPath='C:\Users\sande\Documents\Imperial\matlab\Files\LW41\Load cell data\Lemaire'
fileName=[folderPath,'\Anterior.log']
...
[nextpeakstart,rowpos,FlexExtValues,VarValValues,IntExtValues,AntPosValues,LCValues] = KeyData_v2(VarVal,IntExt,-FlexExt,AntPost,LCdata);
...
DataTable=Write2Excel(ollie_FlexExtValues,ollie_VarValValues,ollie_IntExtValues,ollie_AntPosValues,Output,kneestate,loadtype);
