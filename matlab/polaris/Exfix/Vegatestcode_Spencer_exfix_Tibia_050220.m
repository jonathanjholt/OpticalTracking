...
folderPath='C:\Users\sande\Documents\Imperial\matlab\Files\Exfix_test_tibia';

% TIBIA
%data saved as 3 matrices within XYZdataT

% Medial Malleolus
fileName=[folderPath,'\med_malleolus.csv'];
cal = xlsread(fileName);
XYZdataP(1).points = cal(:,all(~isnan(cal)));

%Lateral Malleolus
fileName=[folderPath,'\lat_malleolus.csv'];
cal = xlsread(fileName);
XYZdataP(2).points = cal(:,all(~isnan(cal)));

%Medial Tibial Epicondyle
fileName=[folderPath,'\med_tib_epicondyle.csv'];
cal = xlsread(fileName);
XYZdataP(3).points = cal(:,all(~isnan(cal)));

%Lateral Tibial Epicondyle
fileName=[folderPath,'\lat_tib_epicondyle.csv'];
cal = xlsread(fileName);
...
%for proximal end
[gTt0, originT, APAxisT, PDAxisT, MLAxisT] = defineBodyFixedFrameTibia(tib_med,tib_lat,med_mall,lat_mall,proximal, Right);
...
% For distal end
[gTf0, originF, APAxisF, PDAxisF, MLAxisF] = defineBodyFixedFrameTibia(tib_med,tib_lat,med_mall,lat_mall,distal, Right);
grf0 = [originF, 1]';
...
folderPath='C:\Users\sande\Documents\Imperial\matlab\Files\Exfix_test_tibia';
fileName=[folderPath,'\Stryker_ex_fix_test.csv'];
...
[ProxDist_max,ProxDistmax_locs]=findpeaks(ollie_ProxDist,time,'MinPeakDistance',0.9);

[ProxDist_min,ProxDistmin_locs]=findpeaks(-ollie_ProxDist,time,'MinPeakDistance',0.9);


%remove beginning and ending with no loading (removing noise)
noise = find(abs(ProxDist_max-ollie_ProxDist(1))<0.3);
