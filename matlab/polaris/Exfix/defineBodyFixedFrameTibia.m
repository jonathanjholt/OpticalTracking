function [ gTf0, origin, j_,k_,i_] = defineBodyFixedFrameTibia(med,lat,med_mal,lat_mal,frac_origin,right)
...
if right
    i_ = uvector(med_mal,lat_mal); %RIGHT KNEE, x Axis
else
    i_ = uvector(lat_mal,med_mal); %LEFT KNEE, x Axis
end
origin = frac_origin;
epi_centre=(med+lat)/2;
mal_centre=(med_mal+lat_mal)/2;
k_=uvector(mal_centre,epi_centre);
j_=ucross(i_,k_);
...
gTf0=trans*rot; %Note this is the same as gTt0=[rot,origin';0 0 0 1];
